...
const ClientsGraph: React.SFC<ClientsGraphProps> = ({ branches }) => {
  const theme = useTheme()

  const annualData: DataProps[] = [
    {
      name: 'Jan',
      count: 3000,
    },
  ...
  const [textRange, setRange] = useState('THIS YEAR')

  const [textBranch, setBranch] = useState('ALL BRANCHES')

  const [titleRange, setTitleRange] = useState(
    new Date(Date.now()).getFullYear().toString()
  )

  const [titleBranch, setTitleBranch] = useState('ALL BRANCHES')
  ...
  return (
    <>
      ...
      <ResponsiveContainer width='100%' height={200}>
        <AreaChart
          data={textRange === 'THIS YEAR' ? annualData : monthlyData}
      ...
      <Grid xs={12} item container alignItems='center' justify='center'>
        <Typography component='h5' variant='subtitle1' color='textPrimary'>
          {'A total of 3000 clients from ' + titleRange}
        </Typography>
      </Grid>
    </>
  )
